 ...
 *
 * DND-API-CROSS-BROWSER-ISSUES:
 *
 * Firefox:
 * - If you set `effectAllowed` or `dropEffect` in dragstart you need to set them in `dragenter/dragover` also
 * - You need to set some data in `dragstart` via `event.dataTransfer.setData(type, data)`
 *
 * InternetExplorer 11:
 * - On MS Surface/Touch you have to touch and hold for a small amount of time to initiate a drag operation.
 * - If you have an event handler for `dragenter` on the body you MUST call `event.preventDefault()` to allow the drag operation,
 *   otherwise it will be aborted immediately.
 * - Handling `dragenter` with `event.preventDefault()` seems not required when dragging over a child element, even when it has an event handler registered.
 * - Scrolls automatically when dragging close to an edge
 *
 * Chrome:
 * - Scrolls automatically when dragging close to an edge
 *
 * Chrome on Android:
 * - You need to handle the `dragenter` event with `event.preventDefault()` or have a valid `dropzone`-attribute on your drop target.
 *
 * Polyfill:
 * - You need to handle the `dragenter` event with `event.preventDefault()` or have a valid `dropzone`-attribute on your drop target.
 */
module MobileDragAndDropPolyfill {
     ...
     * at this point you have the ability to pass
     * a config.
     * @param config
     * @constructor
     */
    ...
    class DragAndDropInitializer {
         ...
         * @type {{log: (function()), dragImageClass: null, iterationInterval: number, coordinateSystemForElementFromPoint: string}}
         */
        ...
        /**
         * Feature detection if environment supports drag and drop or we have to apply shim.
         ...
         * -----------------------------------------------------------------------------------------------------------------------------------------------------
         * Chrome                           Native                  Chrome supports touch devices/events. When run on a desktop touch device like MS Surface
         ...
         * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
         * -
         * Chrome on iOS                    Polyfill                //TODO analyze strange behaviour on ipad on drag/touchend/cancel when close to the right
         * edge
         * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
         * -
         * Chrome on Android                Polyfill                //TODO analyze strange behaviour on ipad on drag/touchend/cancel when close to the right
         * edge
         * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
         * -
         * Chrome on touch capable          Needs polyfill          //TODO How can we detect that a desktop chrome is in touch mode?
         * device
         * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
         * -
         ...
         * -----------------------------------------------------------------------------------------------------------------------------------------------------
         */
        private static IsDragAndDropSupportedNatively():boolean {
        ...
        private static OnTouchstart( e:TouchEvent ) {
            ...
            // TODO revisit if this is the right way to do this here
         ...
         */
        private static DragOperationEnded() {
    ...
    class DragOperationController {
        ...
        private dragDataStore:DragDataStore = new DragDataStore();
        ...
        // helper flags set by the d'n'd iteration
        private dragOperationEnded:boolean = false;
        private dragOperationCancelled:boolean = false;

        constructor( private config:InternalConfig, private sourceNode:Element, initialEvent:TouchEvent, private dragOperationEndedCb:()=>void ) {
            ...
            //TODO the dragstart should be called when movement starts by receiving the first touch move that overcomes a threshold, otherwise a click event
            // should be fired
        ...
        //TODO is this still valid? should the logic to get the drag source element be applied elsewhere and the "click" generation be handled after detecting
        // NO drag operation because not enough movement was applied?
        public static IsDragOperation( event:TouchEvent, config:InternalConfig ):Element {
            ...
            do {
                ...
                if( el.tagName == "A" && !el.getAttribute( "draggable" ) ) {
                    ...
                    // when the event on the anchor is prevented we continue with drag and drop
                    if( !cancelled ) {
                        event.preventDefault();
                        return null;
                    }
        ...
        /**
         * according to http://www.w3.org/TR/2011/WD-html5-20110113/dnd.html#drag-and-drop-processing-model
         */
        private dragAndDropProcessModelIteration():void {
            ...
            // and if this immediate user selection is not the same as the current target element, then update the current target element as follows:
         ...
         * according to http://www.w3.org/TR/2011/WD-html5-20110113/dnd.html#drag-and-drop-processing-model
         */
        private DragOperationEnded( cancelled?:boolean ):boolean {
        ...
        /**
         * Implements http://www.w3.org/TR/2011/WD-html5-20110113/dnd.html#the-dropzone-attribute
         *
         * @param element
         * @param dragDataStore
         * @returns {any}
         * @constructor
         */
        public static GetOperationForMatchingDropzone( element:HTMLElement, dragDataStore:DragDataStore ):string {
         ...
         * @returns {boolean}
         */
        private dragenter( targetElement:Element ):boolean {
            ...
            //TODO does currentDropTarget make sense for relatedTarget here?
         ...
         * @returns {boolean}
         */
        private dragleave( targetElement:Element ):boolean {
    ...
    /**
     * Polyfills http://www.w3.org/TR/2011/WD-html5-20110113/dnd.html#the-datatransfer-interface
     *
     * Does not implement it strictly because File types are not supported.
     ...
     * the data store from the data transfer instance.
     */
    class DataTransfer {
    ...
    /**
     * Polyfills http://www.w3.org/TR/2011/WD-html5-20110113/dnd.html#the-drag-data-store
     */
    class DragDataStore {
